...
func Example() {
	...
	var requestValidator = validator.Struct(func(s validator.StructFieldAdder, r *Request) {
		s.Add(validator.Field(&r.User, "user", validator.Struct(func(s validator.StructFieldAdder, u *User) {
...
func Example_separated() {
	...
	var (
		userValidator = validator.Struct(func(s validator.StructFieldAdder, u *User) {
			s.Add(validator.Field(&u.ID, "id", validator.Length[string](5, 10)))
			s.Add(validator.Field(&u.Name, "name", validator.Required[string]()))
		})
		requestValidator = validator.Struct(func(s validator.StructFieldAdder, r *Request) {
			s.Add(validator.Field(&r.User, "user", userValidator))
			s.Add(validator.Field(&r.Options, "options", validator.Slice[string](
				validator.In("option1", "option2")),
			))
			s.Add(validator.Field(&r.Options, "options", validator.Slice(
				validator.In("option1", "option2"),
			)))
		})
	)
